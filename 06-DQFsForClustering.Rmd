...
row.col <- function(dataframe,index){
  ...
  mi <- index+n.row
  col <- floor(mi/n.row); row <- mi-col*n.row
...
closest.pt.clusters <- function(inter.dists, cg1, cg2){
  n.cg1 <- length(c(cg1))
  n.cg2 <- length(c(cg2))
  min.index <- which.min(inter.dists[cg1,cg2])
...
```{r}
plot.subset <- function(dqf.s,clus.pt,test.pt){
...
```{r}
combine.prompt(5,6,inter.d,combined.clusters)
```
...
dqf.clustering <- function(data = NULL,n.clusters=20, gram.mat = NULL, g.scale=2, angle=c(45), kernel="linear", p1=1, p2=0, n.splits=100, subsample=50, z.scale=TRUE, k.w=3, adaptive=TRUE, G="norm"){
  ...
  while(s != 'exit'){
    ...
    cluster.group1 <- rc$row; cluster.group2 <- rc$col
    
    # Prepare dqfs
    cpc <- closest.pt.clusters(inter.dists,combined.clusters[[row]],combined.clusters[[col]])
    closest.pts[cpc$c1,cpc$c2] <- pt2.index # index of closest point in cluster group 2 to cg1
    closest.pts[cpc$c2,cpc$c1] <- pt1.index # index of closest point in cluster group 1 to cg2
    
    subset1 <- c(all.indices(clusters,combined.clusters[[cluster.group1]]),pt2.index)
    ed1 <- extract.depths(dqf.s,subset1)
    depths1 <- ed1$depths; subset1.pairs <- ed1$subset.pairs
    qfs1 <- calculate.qfs(depths1,dqf.s$k.to.mids,dqf.s$splits)
    dqfs1 <- calculate.dqfs(qfs1,subset1.pairs,subset1,n.obs)
    labels1 <- rep(1,length(subset1)); labels1[which(subset1==pt2.index)] <- 2
    
    subset2 <- c(all.indices(clusters,combined.clusters[[cluster.group2]]),pt1.index)
    ed2 <- extract.depths(dqf.s,subset2)
    depths2 <- ed2$depths; subset2.pairs <- ed2$subset.pairs
    qfs2 <- calculate.qfs(depths2,dqf.s$k.to.mids,dqf.s$splits)
    ...
    if(s == "Yes"){
      ...
      print("Are you sure? Combinning clusters {cluster.string(combined.clusters[[row]])} and {cluster.string(combined.clusters[[col]])} will not be revisited.")
    }else if(s == "Later"){
      print("later")
    }else if(s == row){
      print(row)
